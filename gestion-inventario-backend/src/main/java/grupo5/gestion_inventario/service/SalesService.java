...
public class SalesService {
    ...
    public List<SaleDto> findByClientId(Long clientId) {
        ...
        // 1. Verificar que el BusinessAccount exista
        if (!businessAccountRepo.existsById(clientId)) {
            throw new IllegalArgumentException("BusinessAccount no encontrado: " + clientId);
        }

        // 2. Consultar las ventas del cliente y mapearlas a DTOs
        return saleRepo.findByBusinessAccountId(clientId)   // ← cambio de nombre aquí
                .stream()
                .map(this::toDto)
